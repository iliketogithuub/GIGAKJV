...
Translate the following KJV text for {book_name} Chapter {chapter_num} into the Based King James Version (BKJV) according to these rules. Preserve the verse numbering at the start of each line:
{text}
"""
...
def process_file(source_path, target_path, api_key=None):
    ...
    # Add main book title
    new_content_parts.append(f"# {book_name} - Based King James Version (BKJV)\n")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Translate KJV Markdown Bible to Based King James Version (BKJV)")
    parser.add_argument("--source", required=True, help="Path to source KJV file")
    parser.add_argument("--target", required=True, help="Path to output BKJV file")
